...
async function up() {
  console.log('🔄 Adding Rice Hamali System...');

  try {
    // Create rice_hamali_rates table
    await sequelize.query(`
      CREATE TABLE IF NOT EXISTS rice_hamali_rates (
        id SERIAL PRIMARY KEY,
        work_type VARCHAR(100) NOT NULL,
        work_detail VARCHAR(200) NOT NULL,
        rate_18_21 DECIMAL(10,2) NOT NULL DEFAULT 0,
        rate_21_24 DECIMAL(10,2) NOT NULL DEFAULT 0,
        rate_24_27 DECIMAL(10,2) NOT NULL DEFAULT 0,
    ...
    `);
    console.log('✅ Rice hamali rates table created');

    // Create rice_hamali_entries table
    await sequelize.query(`
    ...
    `);
    console.log('✅ Rice hamali entries table created');

    // Create indexes safely (only if table/columns exist)
    try {
      await sequelize.query(`CREATE INDEX IF NOT EXISTS idx_rice_hamali_rates_work_type ON rice_hamali_rates(work_type)`);
    } catch (e) { /* ignore */ }

    // Only create arrival_id index if the column exists
    try {
      const [columns] = await sequelize.query(`
        SELECT column_name FROM information_schema.columns 
        WHERE table_name = 'rice_hamali_entries' AND column_name = 'arrival_id'
      `);
      if (columns.length > 0) {
        await sequelize.query(`CREATE INDEX IF NOT EXISTS idx_rice_hamali_entries_arrival ON rice_hamali_entries(arrival_id)`);
      }
    } catch (e) { /* ignore */ }

    try {
      await sequelize.query(`CREATE INDEX IF NOT EXISTS idx_rice_hamali_entries_status ON rice_hamali_entries(status)`);
    } catch (e) { /* ignore */ }

    try {
      await sequelize.query(`CREATE INDEX IF NOT EXISTS idx_rice_hamali_entries_batch ON rice_hamali_entries(batch_number)`);
    } catch (e) { /* ignore */ }

    console.log('✅ Rice hamali indexes created');

    // Check if data already exists
    const [existingRates] = await sequelize.query('SELECT COUNT(*) as count FROM rice_hamali_rates');

    if (existingRates[0].count == 0) {
      // Insert rice hamali rates data - Complete 69 work types from user's images
      const ratesData = [
        // Nitt/Shifting
      ...
      console.log('✅ Rice hamali rates data inserted');
    } else {
      console.log('ℹ️ Rice hamali rates data already exists, skipping insert');
    }

    console.log('✅ Rice Hamali System migration completed successfully');

  } catch (error) {
    console.error('❌ Error in Rice Hamali System migration:', error);
    throw error;
  }
}

async function down() {
  console.log('🔄 Removing Rice Hamali System...');

  try {
    await sequelize.query('DROP TABLE IF EXISTS rice_hamali_entries CASCADE');
    await sequelize.query('DROP TABLE IF EXISTS rice_hamali_rates CASCADE');
    console.log('✅ Rice Hamali System tables dropped');
  } catch (error) {
    console.error('❌ Error removing Rice Hamali System:', error);
    throw error;
  }
}
