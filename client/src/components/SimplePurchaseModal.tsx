...
const SimplePurchaseModal: React.FC<SimplePurchaseModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  // Set default variety when riceVarieties are loaded
  useEffect(() => {
    if (riceVarieties.length > 0 && !formData.variety) {
      setFormData(prev => ({ ...prev, variety: riceVarieties[0].name }));
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({
        date: new Date().toISOString().split('T')[0],
        billNumber: '',
        variety: riceVarieties.length > 0 ? riceVarieties[0].name : '',
        bags: '',
        bagSize: '26.00',
        packaging: '',
        from: '',
