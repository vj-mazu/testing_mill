...
const EnhancedPaltiModal: React.FC<EnhancedPaltiModalProps> = ({ isOpen, onClose, onSuccess }) => {
    ...
    // Fetch packagings and locations + lock body scroll
    useEffect(() => {
        if (isOpen) {
            fetchPackagings();
            fetchLocations();
    ...
    const validate = (): boolean => {
        ...
        if (!variety) newErrors.variety = 'Select variety';
        ...
        try {
            ...
            for (let index = 0; index < validTargets.length; index++) {
                ...
                await axios.post('/rice-stock-management/movements', {
                    ...
                    locationCode: locationCode, // Source location
                    toLocation: target.targetLocationCode || locationCode, // Target location
