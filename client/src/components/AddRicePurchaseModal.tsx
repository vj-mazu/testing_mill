...
const AddRicePurchaseModal: React.FC<Props> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error adding rice purchase:', error);
      toast.error(error.response?.data?.error || 'Failed to add rice purchase');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <ModalOverlay onClick={onClose}>
      <ModalContent onClick={(e) => e.stopPropagation()}>
        <ModalHeader>
          <Title>📦 Add Rice Purchase</Title>
          <CloseButton onClick={onClose}>×</CloseButton>
        </ModalHeader>

        <InfoBox>
          <strong>Purchase Entry:</strong> This will add rice stock to your inventory. 
          Select the product type, packaging, and location where the rice will be stored.
        </InfoBox>
        ...
        <form onSubmit={handleSubmit}>
          <FormGrid>
            ...
            <FormGroup>
              ...
              <Input
                ...
                placeholder="Supplier/Source location"
              />
            </FormGroup>
          ...
          <ButtonGroup>
            ...
            <Button type="submit" className="primary" disabled={saving}>
              {saving ? 'Adding Purchase...' : 'Add Purchase'}
            </Button>
          </ButtonGroup>
        </form>
      </ModalContent>
...
export default AddRicePurchaseModal;