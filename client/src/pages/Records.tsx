              ...
              <ExcelTable style={{ minWidth: '1500px' }}>
                ...
                <tbody>
                  {(() => {
                    ...
                    // First pass: group by bill number to count rows
                    processedData.forEach((item: any, i: number) => {
                      if (item.movementType === 'sale' && item.billNumber) {
                        const billKey = `${item.billNumber}_${item.date}`;
            ...
            const processRiceStockData = (rawData: any[]) => {
              ...
              sortedDates.forEach((date, dateIndex) => {
                ...
                if (bifurcationSources.length > 0) {
                  ...
                  dayData.conversions = allGroups.filter(g => g.isPalti || g.movementType === 'palti-shortage');
                                ...
                                <div key={productType}>
                                  <div style={{
                                    ...
                                    <div style={{ padding: '8px 12px' }}>
                                      {/* Conversions / Palti (Before Opening Stock) */}
                                      {dayData.conversions?.filter((item: any) => item.category === productType).length > 0 && (
                                        <div style={{ marginBottom: '12px' }}>
                                          <div style={{
                                            ...
                                            🔄 Shifting / Conversions (Palti)
                                          </div>
                                          {dayData.conversions.filter((item: any) => item.category === productType).map((item: any, idx: number) => (
                                            <div key={`conversion-${productType}-${idx}`} style={{
                                              display: 'grid',
                                              gridTemplateColumns: '60px 80px 1fr 120px 100px 60px',
                                              gap: '8px',
                                              padding: '4px 8px',
                                              ...
                                              <div style={{ textAlign: 'center', fontSize: '8.5pt', fontWeight: 'bold' }}>
                                                {item.qtls.toFixed(2)}
                                              </div>
                                              <div style={{ textAlign: 'center', fontSize: '8.5pt' }}>
                                                {item.bags}{item.bagSizeKg ? `/${item.bagSizeKg}kgs` : ''}
                                              </div>
                                              <div style={{ fontSize: '8.5pt', textAlign: 'center' }}>
                                                {item.product}
                                                {item.shortageKg > 0 && (
                                                  <span style={{ color: '#dc2626', fontWeight: 'bold', marginLeft: '4px' }}>
                                                    (Shortage: {item.shortageKg}kg)
                                                  </span>
                                                )}
                                              </div>
                                              <div style={{ fontSize: '8.5pt', textAlign: 'center' }}>{item.variety || 'Unknown'}</div>
                                              <div style={{ fontSize: '8.5pt', textAlign: 'center', color: '#6d28d9' }}>
                                                {item.packaging}
                                              </div>
                                              <div style={{ fontSize: '8.5pt', textAlign: 'center' }}>{item.location}</div>
                                            </div>
                                          ))}
                                          <div style={{ borderBottom: '1px dashed #7c3aed', margin: '8px 0' }}></div>
                                        </div>
                                      )}

                                      {/* Yesterday's Bifurcation (Opening Stock Detailed) */}
                                      {dayData.yesterdayBifurcation?.filter((item: any) => item.category === productType).length > 0 && (
                                        <div style={{
                                          fontSize: '8.5pt',
                                          fontWeight: 'bold',
                                          color: '#4b5563',
                                    ...
                                    <div style={{ padding: '4px 8px' }}>
                                      ...
                                      {dayData.conversions?.filter((item: any) => item.category === productType).map((item: any, idx: number) => (
                                        <div key={`conversion-${productType}-${idx}`} style={{
                                          ...
                                          <div style={{ textAlign: 'center', fontWeight: 'bold' }}>{item.qtls.toFixed(2)}</div>
                                          <div style={{ textAlign: 'center' }}>{item.bags}/{item.bagSizeKg}</div>
                                          <div style={{ textAlign: 'center' }}>🔄 Palti</div>
                                          <div style={{ textAlign: 'center' }}>{item.variety}</div>
                                          <div style={{ textAlign: 'center', fontStyle: 'italic' }}>{item.packaging}</div>
