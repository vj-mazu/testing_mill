                                ...
                                {(() => {
                                  ...
                                  return (
                                    <>
                                      ...
                                      {/* Excludes CLEARING entries which are shown separately below */}
                                      {(() => {
                                        // Group rice productions by outturn (excluding CLEARING entries)
                                        const riceDeductionByOutturn: { [key: string]: { bags: number; variety: string; outturnCode: string } } = {};

                                        todayRiceProductions
                                          .filter((rp: any) => {
                                            // Filter out CLEARING entries - they are displayed separately
                                            if (rp.locationCode === 'CLEARING') return false;
                                      ...
                                      {/* CLEAR OUTTURN Entries - DARK MAROON - SHOWN SEPARATELY (UNIQUE) */}
                                      {(() => {
                                        // Get CLEARING entries for today - each shown as unique row
                                        const clearingEntries = todayRiceProductions.filter((rp: any) => 
                                          rp.locationCode === 'CLEARING' && rp.paddyBagsDeducted > 0
                                        );

                                        return clearingEntries.length > 0 && (
                                          <table style={{
                                            width: '100%',
                                            borderCollapse: 'collapse',
                                            fontFamily: 'Calibri, sans-serif',
                                            ...
                                            <tbody>
                                              {clearingEntries.map((rp: any, idx: number) => {
                                                const outturnCode = rp.outturn?.code || 'UNKNOWN';
                                                const deductedBags = rp.paddyBagsDeducted || 0;
                                                ...
                                                if (rp.outturn?.allottedVariety) {
                                                  ...
                                                  if (outturnArrival) {
                                                    variety = (outturnArrival as any).variety || 'Unknown';
                                                  }
                                                }
                                                ...
                                                return (
                                                  <tr key={`clearing-${idx}`}>
                                                    ...
                                                    <td style={{
                                                      ...
                                                      onClick={() => {
                                                        const outturn = outturns.find((o: any) =>
                                                          (o.outturnNumber === outturnCode) || (o.code === outturnCode)
                                                        );
                                                        if (outturn) {
                                                          setSelectedOutturnId(outturn.id.toString());
                                                          setActiveTab('outturn-report');
                                                    ...
                                                    >
                                                      ✅ {outturnCode} → Clear Outturn (Closed)
                                                    </td>
                                                  </tr>
                                                );
                                              })}
